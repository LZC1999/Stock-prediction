...
data = pd.read_csv("day_train_data.csv", encoding='gbk').drop('交易日期', axis=1).values
...
model = keras.Sequential([
    keras.layers.LSTM(units=128, activation='tanh', return_sequences=True),  # 隐层
    keras.layers.Dropout(0.2),
    keras.layers.LSTM(units=60, return_sequences=True),  # 隐层
    keras.layers.LSTM(units=32),  # 隐层
